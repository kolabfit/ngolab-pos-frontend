(function ($) {
	...
	var dlabChartlist = function () {
		...
		const chartData = {
			labels: ['Income Senin', 'Income Selasa', 'Income Rabu', 'Income Kamis', 'Income Jumat'],
			series: [
				[5, 4, 3, 7, 5],
				[3, 2, 9, 5, 4],
				[1, 5, 8, 4, 6]
		...
		const defaultDataChart = () => {
			const chart = new Chartist.Bar('#default-data', chartData, chartOptions, responsiveOptions);

			// Draw event optimization
			chart.on('draw', data => {
				if (data.type === 'bar') {
					data.element.attr({ style: 'stroke-width: 4%' });
				}
			});
		};
