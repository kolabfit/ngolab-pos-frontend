...
<body data-page="index.php">
	...
	***********************************-->
		...
		document.addEventListener("DOMContentLoaded", function () {
			async function fetchBestCategories() {
				try {
					...
					if (!response.ok) {
						throw new Error('Gagal memuat data kategori terlaris');
					}
	...
	<script>
		// URL API Anda
		const apiUrl = '/outlet/transactions/sales/weekly'; // Ganti dengan URL API Anda
	...
	<script>
		...
		async function fetchSalesData() {
			try {
				...
				const response = await fetch('/transactions/sales/day');
	...
	<script>
		...
		fetchData('/transactions/total/hour');


		document.getElementById('turnOnMonthly').addEventListener('change', async function () {
			let url;
			if (this.checked) {
				url = `/transactions/total/hour?monthly=true`;
			} else {
				url = `/transactions/total/hour`
			}
